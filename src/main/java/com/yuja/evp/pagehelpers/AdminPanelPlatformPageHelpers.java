...
public class AdminPanelPlatformPageHelpers extends Helpers {
	...
	public void fillMetadataInfo(String metadataName, String metadataType, Boolean requirement) {
		...
		if (metadataType == "List") { addDefaultListValue(lastRowIndex+1);}
	...
	public void verifyMetadataAddedToAdminPanel(Boolean requirement) {
		...
		if (requirement == true) {
		WebElement requiredStatus = driver.findElement(By.cssSelector("table>tbody>tr:nth-child("+ lastRow +")>td:nth-child(3)>div>div>p"));
		System.out.println(requiredStatus.getText());
		if ((requiredStatus.getText()).equals("Yes")) {reportStep("Requirement Set to Yes", "PASS", false);} 
	...
	public void deleteMetadata(String metadataName, String metadataType) throws InterruptedException {
		...
		if(metadataType == "List") { clickElement(metadataEntry,"Click Delete option", By.cssSelector("table>tbody>tr:nth-child("+ i +")>td:nth-child(4)>div>div>div>ul>li:nth-child(2)>a"),3);  }
		else {clickElement(metadataEntry,"Click Delete option", By.cssSelector("table>tbody>tr:nth-child("+ i +")>td:nth-child(4)>div>div>div>ul>li>a"),3);}
		List <WebElement> modalList = driver.findElements(By.className("modal-content"));
